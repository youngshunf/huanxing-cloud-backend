...
class HasnMessagesSchemaBase(SchemaBase):
    ...
    from_type: int = Field(description='发送方类型 (1:人类/2:代理/3:系统)')
    to_id: str = Field(description='接收方 hasn_id')
    to_type: int = Field(description='接收方类型 (1:人类/2:代理/3:系统)')
    content_type: int = Field(description='内容类型 (1:文本/2:图片/3:文件/4:语音/5:卡片/6:能力请求/7:能力响应)')
    content: dict = Field(description='消息内容 (JSON: {text/url/filename/...})')
    msg_type: str = Field(description='消息类型 (message:普通消息/contact_request:好友请求/contact_accept:接受好友/contact_reject:拒绝好友/notification:通知/system:系统消息)')
    ...
    context: dict | None = Field(None, description='消息上下文 (JSON: relation_type/scope/trade_session_id/thread_id 等)')
