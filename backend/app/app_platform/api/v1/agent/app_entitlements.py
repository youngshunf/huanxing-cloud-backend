...
async def agent_list_app_entitlementss(
    db: CurrentSession,
    request: Request) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent

    user_id = agent.owner_user_id
    data = await app_entitlements_service.get_list(db=db)
...
async def agent_create_app_entitlements(
    db: CurrentSessionTransaction,
    request: Request,
    obj: CreateAppEntitlementsParam) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_get_app_entitlements(
    db: CurrentSession,
    request: Request,
    pk: Annotated[int, Path(description='App 购买凭证 ID')]) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_update_app_entitlements(
    db: CurrentSession,
    request: Request,
    pk: Annotated[int, Path(description='App 购买凭证 ID')],
    obj: UpdateAppEntitlementsParam) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_delete_app_entitlements(
    db: CurrentSession,
    request: Request,
    pk: Annotated[int, Path(description='App 购买凭证 ID')]) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
