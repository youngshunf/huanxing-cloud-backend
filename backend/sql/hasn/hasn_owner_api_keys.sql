-- =====================================================
-- HASN Owner API Key 表（S0/S1 codegen 输入）
-- =====================================================
CREATE TABLE "public"."hasn_owner_api_keys" (
  ...
  CONSTRAINT "uq_hasn_owner_api_keys_key" UNIQUE ("key_id")
);

CREATE INDEX "idx_hasn_owner_api_keys_owner" ON "public"."hasn_owner_api_keys" ("owner_id");
CREATE INDEX "idx_hasn_owner_api_keys_user" ON "public"."hasn_owner_api_keys" ("user_id") WHERE "user_id" IS NOT NULL;
CREATE INDEX "idx_hasn_owner_api_keys_status" ON "public"."hasn_owner_api_keys" ("status");
CREATE INDEX "idx_hasn_owner_api_keys_bound_node" ON "public"."hasn_owner_api_keys" ("bound_node_id") WHERE "bound_node_id" IS NOT NULL;
CREATE INDEX "idx_hasn_owner_api_keys_sync_revision" ON "public"."hasn_owner_api_keys" ("sync_revision");
...
COMMENT ON COLUMN "public"."hasn_owner_api_keys"."sync_revision" IS '服务端同步修订号';
