...
-- 自动生成于: 2026-03-22 17:39:14.782467+08:00
...
DECLARE
    ...
    v_menu_id INTEGER;
BEGIN
    -- 查找或创建父级目录菜单 (path = /hasn_core)
    SELECT id INTO v_parent_id FROM sys_menu 
    WHERE path = '/hasn_core' AND type = 0
    ORDER BY id LIMIT 1;
    
    IF v_parent_id IS NULL THEN
        INSERT INTO sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
        VALUES ('', 'Hasn_core', '/hasn_core', 1, 'lucide:folder', 0, 'BasicLayout', NULL, 1, 1, 1, '', 'hasn_core模块', NULL, NOW(), NULL)
        RETURNING id INTO v_parent_id;
    END IF;

    -- 查找或创建主菜单 (path = /hasn_core/hasn_humans)
    SELECT id INTO v_menu_id FROM sys_menu 
    WHERE path = '/hasn_core/hasn_humans' AND type = 1
    ORDER BY id LIMIT 1;
    
    IF v_menu_id IS NULL THEN
        INSERT INTO sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
        VALUES ('', 'HasnHumans', '/hasn_core/hasn_humans', 1, 'lucide:list', 1, '/hasn_core/hasn_humans/index', NULL, 1, 1, 1, '', 'HASN Human 用户', v_parent_id, NOW(), NULL)
