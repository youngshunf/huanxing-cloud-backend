"""HASN 客户端设备 - 用户端 API

认证方式: DependsJwtAuth（仅当前登录用户）
数据隔离: 通过 request.user.id 限制为用户自己的数据
"""
from typing import Annotated

from fastapi import APIRouter, Path, Request

from backend.app.hasn_core.schema.hasn_clients import (
    CreateHasnClientsParam,
    GetHasnClientsDetail,
    UpdateHasnClientsParam,
)
from backend.app.hasn_core.service.hasn_clients_service import hasn_clients_service
from backend.common.exception import errors
from backend.common.pagination import DependsPagination, PageData
from backend.common.response.response_schema import ResponseModel, ResponseSchemaModel, response_base
from backend.common.security.jwt import DependsJwtAuth
from backend.database.db import CurrentSession, CurrentSessionTransaction

router = APIRouter()


@router.get(
    '',
    summary='获取我的HASN 客户端设备列表',
    dependencies=[DependsJwtAuth, DependsPagination],
)
async def get_my_hasn_clientss(
    request: Request,
    db: CurrentSession,
) -> ResponseSchemaModel[PageData[GetHasnClientsDetail]]:
    user_id = request.user.id
    page_data = await hasn_clients_service.get_list(db=db, user_id=user_id)
    return response_base.success(data=page_data)


@router.post(
    '',
    summary='创建HASN 客户端设备',
    dependencies=[DependsJwtAuth],
)
async def create_my_hasn_clients(
    request: Request,
    db: CurrentSessionTransaction,
    obj: CreateHasnClientsParam,
) -> ResponseModel:
    user_id = request.user.id
    result = await hasn_clients_service.create(db=db, obj=obj, user_id=user_id)
    return response_base.success(data=result)


@router.get(
    '/{pk}',
    summary='获取HASN 客户端设备详情',
    dependencies=[DependsJwtAuth],
)
async def get_my_hasn_clients(
    request: Request,
    db: CurrentSession,
    pk: Annotated[int, Path(description='HASN 客户端设备 ID')],
) -> ResponseSchemaModel[GetHasnClientsDetail]:
    hasn_clients = await hasn_clients_service.get(db=db, pk=pk)
    if hasn_clients.user_id != request.user.id:
        raise errors.ForbiddenError(msg='无权访问该HASN 客户端设备')
    return response_base.success(data=hasn_clients)


@router.put(
    '/{pk}',
    summary='更新HASN 客户端设备',
    dependencies=[DependsJwtAuth],
)
async def update_my_hasn_clients(
    request: Request,
    db: CurrentSessionTransaction,
    pk: Annotated[int, Path(description='HASN 客户端设备 ID')],
    obj: UpdateHasnClientsParam,
) -> ResponseModel:
    user_id = request.user.id
    hasn_clients = await hasn_clients_service.get(db=db, pk=pk)
    if hasn_clients.user_id != user_id:
        raise errors.ForbiddenError(msg='无权修改该HASN 客户端设备')
    count = await hasn_clients_service.update(db=db, pk=pk, obj=obj, user_id=user_id)
    if count > 0:
        return response_base.success()
    return response_base.fail()


@router.delete(
    '/{pk}',
    summary='删除HASN 客户端设备',
    dependencies=[DependsJwtAuth],
)
async def delete_my_hasn_clients(
    request: Request,
    db: CurrentSessionTransaction,
    pk: Annotated[int, Path(description='HASN 客户端设备 ID')],
) -> ResponseModel:
    user_id = request.user.id
    hasn_clients = await hasn_clients_service.get(db=db, pk=pk)
    if hasn_clients.user_id != user_id:
        raise errors.ForbiddenError(msg='无权删除该HASN 客户端设备')
    from backend.app.hasn_core.schema.hasn_clients import DeleteHasnClientsParam
    count = await hasn_clients_service.delete(db=db, obj=DeleteHasnClientsParam(pks=[pk]))
    if count > 0:
        return response_base.success()
    return response_base.fail()
