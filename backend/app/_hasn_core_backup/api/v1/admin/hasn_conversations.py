...
from backend.app.hasn_core.schema.hasn_conversations import (
    CreateHasnConversationsParam,
    DeleteHasnConversationsParam,
    GetHasnConversationsDetail,
    UpdateHasnConversationsParam,
)
from backend.app.hasn_core.service.hasn_conversations_service import hasn_conversations_service
...
@router.get('/{pk}', summary='获取HASN 对话/会话详情', dependencies=[DependsJwtAuth])
async def get_hasn_conversations(
    db: CurrentSession, pk: Annotated[int, Path(description='HASN 对话/会话 ID')]
...
async def update_hasn_conversations(
    db: CurrentSessionTransaction, pk: Annotated[int, Path(description='HASN 对话/会话 ID')], obj: UpdateHasnConversationsParam
) -> ResponseModel:
