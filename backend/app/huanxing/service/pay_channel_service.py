...
class PayChannelService:
    ...
    @staticmethod
    async def get_list(db: AsyncSession, app_id: int | None = None) -> dict[str, Any]:
        select_stmt = await pay_channel_dao.get_select(app_id=app_id)
        return await paging_data(db, select_stmt)

    @staticmethod
    async def get_active_channels(*, db: AsyncSession, app_id: int) -> Sequence[PayChannel]:
        """获取启用的渠道列表（用户端展示用，不含密钥）"""
        return await pay_channel_dao.get_active_by_app(db, app_id)

    @staticmethod
    async def create(*, db: AsyncSession, obj: CreatePayChannelParam) -> None:
        existing = await pay_channel_dao.get_by_app_and_code(db, obj.app_id, obj.code)
        if existing:
