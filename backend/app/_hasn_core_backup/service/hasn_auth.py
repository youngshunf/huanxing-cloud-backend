...
async def register_hasn_identity(
    ...
    返回: {human: HasnHumans, agent: HasnAgents, api_key: str}
    """
    # 检查是否已注册
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(status_code=409, detail='该用户已注册 HASN 身份')
    ...
    human = HasnHumans(
        ...
        stats={'contacts_count': 0, 'agents_count': 1},
    )
    db.add(human)
    await db.flush()
