...
@router.get(
    '',
    summary='获取我的App Tool 定义列表',
    dependencies=[DependsJwtAuth, DependsPagination],
)
async def get_my_app_toolss(
    request: Request,
    db: CurrentSession,
...
async def get_my_app_tools(
    ...
    if app_tools.user_id != request.user.id:
...
async def update_my_app_tools(
    ...
    app_tools = await app_tools_service.get(db=db, pk=pk)
    if getattr(app_tools, 'user_id', request.user.id) != request.user.id:
