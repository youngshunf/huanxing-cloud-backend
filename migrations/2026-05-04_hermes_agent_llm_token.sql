-- 配合 §09 平台 LLM 计费与短期凭证：
--   * 新增 hermes_agent_llm_token 表，落地 per-Agent 凭证隔离记录
--   * 仅唤星侧库新增；不改 new-api 库 schema
-- 关联：
--   * docs/huanxing-hermes-runtime设计文档/09-平台LLM计费与短期凭证.md §2.1
--   * backend/sql/tables/hermes_agent_llm_token.sql（codegen 源）
-- 2026-05-04

CREATE TABLE IF NOT EXISTS "public"."hermes_agent_llm_token" (
...
CREATE INDEX IF NOT EXISTS "idx_hermes_agent_llm_token_user_id"
  ON "public"."hermes_agent_llm_token" ("user_id");
CREATE INDEX IF NOT EXISTS "idx_hermes_agent_llm_token_agent_id"
  ON "public"."hermes_agent_llm_token" ("agent_id");
CREATE INDEX IF NOT EXISTS "idx_hermes_agent_llm_token_newapi_token_id"
  ON "public"."hermes_agent_llm_token" ("newapi_token_id");
