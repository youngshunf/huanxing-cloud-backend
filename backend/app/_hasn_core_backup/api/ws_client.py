...
from backend.app.hasn_core.service.hasn_auth import verify_client_jwt
from backend.app.hasn_core.service.ws_router import ws_router
from backend.app.hasn_core.service import message_router
...
async def _handle_send(
    ...
    reply_to_id = msg.get('reply_to_id')  # 回复/引用的消息 ID
    ...
    sender_payload = {
        ...
        'message': {
            ...
            'status': 1,
            'local_id': local_id,
            'self_sent': True,  # 标记为自己发的，其他客户端用于同步
        },
    }
    # 广播给发送方的其他在线客户端（排除当前客户端）
    from backend.database.redis import redis_client
    other_clients = await redis_client.smembers(f'hasn:user_clients:{user_hasn_id}')
    for cid in other_clients:
        if cid != client_id:
            from backend.app.hasn_core.service.ws_router import _ws_connections
