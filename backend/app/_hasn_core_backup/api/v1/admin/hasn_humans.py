...
from backend.app.hasn_core.schema.hasn_humans import (
    CreateHasnHumansParam,
    DeleteHasnHumansParam,
    GetHasnHumansDetail,
    UpdateHasnHumansParam,
)
from backend.app.hasn_core.service.hasn_humans_service import hasn_humans_service
...
@router.get('/{pk}', summary='获取HASN Human 用户详情', dependencies=[DependsJwtAuth])
async def get_hasn_humans(
    db: CurrentSession, pk: Annotated[int, Path(description='HASN Human 用户 ID')]
...
async def update_hasn_humans(
    db: CurrentSessionTransaction, pk: Annotated[int, Path(description='HASN Human 用户 ID')], obj: UpdateHasnHumansParam
) -> ResponseModel:
