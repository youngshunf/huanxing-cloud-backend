...
async def agent_create_lead_export_item(
    ...
    request: Request,
    obj: CreateLeadExportItemParam) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_get_lead_export_item(
    db: CurrentSession,
    request: Request,
    pk: Annotated[int, Path(description='Lead CSV export item snapshot ID')]) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_update_lead_export_item(
    ...
    obj: UpdateLeadExportItemParam) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
