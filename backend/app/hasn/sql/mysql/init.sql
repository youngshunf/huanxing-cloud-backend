-- =====================================================
--  菜单初始化 SQL (MySQL)
-- 自动生成于: 2026-03-27 17:52:50.790714+08:00
...
-- 查找主菜单 (path = /hasn/hasn_humans)
SET @menu_id = (SELECT id FROM sys_menu WHERE path = '/hasn/hasn_humans' AND type = 1 ORDER BY id LIMIT 1);

-- 如果主菜单不存在，创建它
INSERT INTO sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
SELECT '', 'HasnHumans', '/hasn/hasn_humans', 1, 'lucide:list', 1, '/hasn/hasn_humans/index', NULL, 1, 1, 1, '', 'HASN Human 用户', @parent_id, NOW(), NULL
