-- =====================================================
-- 平台权限定义表（hasn.*管理 菜单初始化 SQL (PostgreSQL)
-- 自动生成于: 2026-05-14 13:06:44.755720
...
BEGIN
    ...
    IF v_menu_id IS NULL THEN
        INSERT INTO sys_menu (title, name, path, sort, icon, type, component, perms, status, display, cache, link, remark, parent_id, created_time, updated_time)
        VALUES ('平台权限定义表（hasn.*管理', 'PlatformScopes', '/app_platform/platform_scopes', 1, 'lucide:list', 1, '/app_platform/platform_scopes/index', NULL, 1, 1, 1, '', '平台权限定义表（hasn.* namespace）', v_parent_id, NOW(), NULL)
        RETURNING id INTO v_menu_id;
    ELSE
        UPDATE sys_menu SET
            title = '平台权限定义表（hasn.*管理',
            name = 'PlatformScopes',
