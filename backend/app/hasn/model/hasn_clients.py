...
class HasnClients(Base):
    """HASN 节点设备表（统一节点模型 v5.0）

    节点仅代表物理设备，不绑定特定用户。
    Human/Agent 通过 report_entities 动态上报。
    """

    __tablename__ = 'hasn_clients'

    id: Mapped[id_key] = mapped_column(init=False)
    client_id: Mapped[str] = mapped_column(sa.String(40), default='', comment='节点唯一标识 (格式: n_{uuid_short})')
    user_hasn_id: Mapped[str | None] = mapped_column(sa.String(40), default=None, comment='初始注册用户（仅用于审计，不用于路由）')
    ...
    api_key_hash: Mapped[str | None] = mapped_column(sa.String(64), default=None, comment='Node Key (hasn_nk_) 的 SHA256 哈希')
