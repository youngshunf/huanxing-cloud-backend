-- =====================================================
-- 社区文章表 字典数据初始化 SQL
-- 自动生成于: 2026-05-22 20:51:40.921112
-- =====================================================

-- author_type 字典类型
INSERT INTO sys_dict_type (name, code, remark, created_time, updated_time)
VALUES
('author_type', 'hasn_author_type', '社区文章表模块-author_type', NOW(), NULL)
...
-- generation_type 字典类型
INSERT INTO sys_dict_type (name, code, remark, created_time, updated_time)
VALUES
('generation_type', 'hasn_generation_type', '社区文章表模块-generation_type', NOW(), NULL)
ON CONFLICT (code) DO UPDATE SET name = EXCLUDED.name, remark = EXCLUDED.remark, updated_time = NOW();

-- generation_type 字典数据
DO $$
DECLARE
    v_dict_type_id INTEGER;
BEGIN
...
INSERT INTO sys_dict_type (name, code, remark, created_time, updated_time)
VALUES
('status', 'hasn_status', '社区文章表模块-status', NOW(), NULL)
ON CONFLICT (code) DO UPDATE SET name = EXCLUDED.name, remark = EXCLUDED.remark, updated_time = NOW();
