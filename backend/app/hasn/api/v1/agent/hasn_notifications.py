...
async def agent_create_hasn_notifications(
    ...
    request: Request,
    obj: CreateHasnNotificationsParam) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_get_hasn_notifications(
    db: CurrentSession,
    request: Request,
    pk: Annotated[int, Path(description='HASN 通知队列 ID')]) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
...
async def agent_update_hasn_notifications(
    ...
    obj: UpdateHasnNotificationsParam) -> ResponseModel:
    agent: AgentTokenPayload = request.state.agent
